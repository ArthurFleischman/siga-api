defmodule Siga.EntitiesTest do
  ...
  describe "pre_requirements" do
    alias Siga.Entities.PreRequirements

    @valid_attrs %{}
    @update_attrs %{}
    @invalid_attrs %{}

    def pre_requirements_fixture(attrs \\ %{}) do
      {:ok, pre_requirements} =
        attrs
        |> Enum.into(@valid_attrs)
        |> Entities.create_pre_requirements()

      pre_requirements
    end

    test "list_pre_requirements/0 returns all pre_requirements" do
      pre_requirements = pre_requirements_fixture()
      assert Entities.list_pre_requirements() == [pre_requirements]
    end

    test "get_pre_requirements!/1 returns the pre_requirements with given id" do
      pre_requirements = pre_requirements_fixture()
      assert Entities.get_pre_requirements!(pre_requirements.id) == pre_requirements
    end

    test "create_pre_requirements/1 with valid data creates a pre_requirements" do
      assert {:ok, %PreRequirements{} = pre_requirements} = Entities.create_pre_requirements(@valid_attrs)
    end

    test "create_pre_requirements/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Entities.create_pre_requirements(@invalid_attrs)
    end

    test "update_pre_requirements/2 with valid data updates the pre_requirements" do
      pre_requirements = pre_requirements_fixture()
      assert {:ok, %PreRequirements{} = pre_requirements} = Entities.update_pre_requirements(pre_requirements, @update_attrs)
    end

    test "update_pre_requirements/2 with invalid data returns error changeset" do
      pre_requirements = pre_requirements_fixture()
      assert {:error, %Ecto.Changeset{}} = Entities.update_pre_requirements(pre_requirements, @invalid_attrs)
      assert pre_requirements == Entities.get_pre_requirements!(pre_requirements.id)
    end

    test "delete_pre_requirements/1 deletes the pre_requirements" do
      pre_requirements = pre_requirements_fixture()
      assert {:ok, %PreRequirements{}} = Entities.delete_pre_requirements(pre_requirements)
      assert_raise Ecto.NoResultsError, fn -> Entities.get_pre_requirements!(pre_requirements.id) end
    end

    test "change_pre_requirements/1 returns a pre_requirements changeset" do
      pre_requirements = pre_requirements_fixture()
      assert %Ecto.Changeset{} = Entities.change_pre_requirements(pre_requirements)
    end
  end
  ...
  describe "requirements" do
    ...
    test "update_requirements/2 with valid data updates the requirements" do
      requirements = requirements_fixture()
      assert {:ok, %Requirements{} = requirements} = Entities.update_requirements(requirements, @update_attrs)
    end

    test "update_requirements/2 with invalid data returns error changeset" do
      requirements = requirements_fixture()
      assert {:error, %Ecto.Changeset{}} = Entities.update_requirements(requirements, @invalid_attrs)
  ...
  describe "requirements" do
    ...
    test "create_requirements/1 with valid data creates a requirements" do
      assert {:ok, %Requirements{} = requirements} = Entities.create_requirements(@valid_attrs)
      assert requirements.type == "some type"
    end
    ...
    test "update_requirements/2 with valid data updates the requirements" do
      requirements = requirements_fixture()
      assert {:ok, %Requirements{} = requirements} = Entities.update_requirements(requirements, @update_attrs)
      assert requirements.type == "some updated type"
    end
